...
def train_model(model, dataloader, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for tuple in dataloader:
            input_ids = tuple[0]
            target_ids = tuple[1]
            batch = torch.cat(input_ids, target_ids)
            print(batch)
        #input_batch = torch.tensor(input_ids)
        #target_batch = torch.tensor(target_ids)
        train_step(model, input_ids, target_ids, optimizer)
    pass
...
losses = train_model(model, dl, optimizer, num_epochs=2)
